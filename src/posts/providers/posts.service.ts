...
export class PostsService {
  ...
  public async findAll(postQuery : GetPostsDto , userId: number) {
   const posts = this.paginationProvider.paginatedQuery(postQuery , this.postRepositry)
   return posts;
  }
  ...
  public async updatePost (patchPostDto : PatchPostDto) {
   ...
   let postExists = undefined; 
    ...
    try {
      postExists = await this.postRepositry.findOneBy({slug : patchPostDto.slug});
    } catch (error) {
    ...
    await this.postRepositry.save(post);
     ...
     throw new RequestTimeoutException("unable to process your request at moment please try again 44")
    }
